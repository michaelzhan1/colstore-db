...
def generateTable1():
    """ Generates data file for first table. """
    file = os.path.join(DATA_DIR, 'data1_generated.csv')
...
def createTestOne():
    with utils.InputFileWriter(1, test_dir=INPUT_DIR) as f:
        ...
        f.write(f'load("{os.path.join(DATA_DIR, "data1_generated.csv")}")\n')
        f.write('shutdown\n')
    
    with utils.ExpectedFileWriter(1, test_dir=EXP_DIR) as exp_file:
        exp_file.write('')  # No expected output for this test


def createTestTwo(table):
    selectValLess = 20
    ...
    with utils.InputFileWriter(2, test_dir=INPUT_DIR) as f:
        ...
        f.write('--\n')
        f.write(f'-- SELECT col1 FROM tbl1 WHERE col1 < {selectValLess};\n')
        f.write(f's1=select(db1.tbl1.col1,null,{selectValLess})\n')
        f.write('f1=fetch(db1.tbl1.col1,s1)\n')
        f.write('print(f1)\n')
        f.write('--\n')
        f.write(f'-- SELECT col2 FROM tbl1 WHERE col1 >= {selectValGreater};\n')
        f.write(f's2=select(db1.tbl1.col1,{selectValGreater},null)\n')
        f.write('f2=fetch(db1.tbl1.col2,s2)\n')
        f.write('print(f2)\n')

    exp1 = table[table['col1'] < selectValLess]['col1']
    exp2 = table[table['col1'] >= selectValGreater]['col2']
...
def createTestThree(dataTable):
    # prelude
    output_file, exp_output_file = utils.openFileHandles(3, test_dir=TEST_BASE_DIR)
...
def generateDataFile2(dataSizeTableTwo):
    ...
    outputTable = pd.DataFrame(np.random.randint(-1 * dataSizeTableTwo/2, dataSizeTableTwo/2,
                               size=(dataSizeTableTwo, 4)), columns=['col1', 'col2', 'col3', 'col4'])
...
def createTestFour(dataTable):
    # prelude
    output_file, exp_output_file = utils.openFileHandles(4, test_dir=TEST_BASE_DIR)
...
def createTestFive(dataTable, dataSizeTableTwo, approxSelectivity):
    # prelude
    output_file, exp_output_file = utils.openFileHandles(5, test_dir=TEST_BASE_DIR)
...
def createTestSix(dataTable, dataSizeTableTwo, approxNumOutputTuples):
    # prelude
    output_file, exp_output_file = utils.openFileHandles(6, test_dir=TEST_BASE_DIR)
...
def createTestSeven(dataTable, dataSizeTableTwo, approxNumOutputTuples):
    output_file, exp_output_file = utils.openFileHandles(7, test_dir=TEST_BASE_DIR)
...
def createTestEight(dataTable, dataSizeTableTwo, approxSelectivity):
    output_file, exp_output_file = utils.openFileHandles(8, test_dir=TEST_BASE_DIR)
...
def createTestNine(dataTable, dataSizeTableTwo, approxSelectivity):
    output_file, exp_output_file = utils.openFileHandles(9, test_dir=TEST_BASE_DIR)
