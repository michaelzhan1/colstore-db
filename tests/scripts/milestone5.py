...
def generateDataMilestone5(dataSize):
    outputFile = os.path.join(DATA_DIR, 'data5.csv')
...
def createTest60(dataTable):
    # prelude
    output_file, exp_output_file = utils.openFileHandles(60)
    ...
    output_file.write('load(\"'+DATA_DIR+'/data5.csv\")\n')
    output_file.write('--\n')
...
def createTest61(dataTable, approxSelectivity):
    output_file, exp_output_file = utils.openFileHandles(61)
    ...
    output_file.write('--\n')
    output_file.write(
        '-- SELECT col3 FROM tbl5 WHERE col1 >= {} AND col1 < {};\n'.format(selectValLess2, selectValGreater2))
    output_file.write('--\n')
...
def createTests62(dataTable):
    output_file, exp_output_file = utils.openFileHandles(62)
...
def createTest63(dataTable):
    output_file, exp_output_file = utils.openFileHandles(63)
...
def createTest64(dataTable):
    output_file, exp_output_file = utils.openFileHandles(64)
...
def createRandomSelects(dataTable, numberOfQueries, output_file, exp_output_file):
    ...
    for i in range(numberOfQueries):
        ...
        output_file.write(
            '-- SELECT col1 FROM tbl5 WHERE col2 >= {} AND col2 < {};\n'.format(selectValLess, selectValGreater))
        output_file.write('s1=select(db1.tbl5.col2,{},{})\n'.format(selectValLess, selectValGreater))
        output_file.write('f1=fetch(db1.tbl5.col1,s1)\n')
        output_file.write('print(f1)\n')
        dfSelectMaskGT = dataTable['col2'] >= selectValLess
...
def createTest65(dataTable):
    output_file, exp_output_file = utils.openFileHandles(65)
    output_file.write(
...
def generateMilestoneFiveFiles(dataSize, randomSeed=0):
    np.random.seed(randomSeed)
    dataTable = generateDataMilestone5(dataSize)
    dataTable = createTest60(dataTable)
    createTest61(dataTable, 0.1)
    dataTable = createTests62(dataTable)
    createTest63(dataTable)
    dataTable = createTest64(dataTable)
    createTest65(dataTable)


def main(argv):
    dataSize = 10000
    generateMilestoneFiveFiles(dataSize)
